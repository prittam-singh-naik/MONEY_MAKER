        ...
        try {
                swingListNew.forEach((swing, index) => {
                        ...
                        if (inBetweenSwings.length > 1) {
                                ...
                                const currentSwingCondition = swing.reversedFrom.includes('0.618') && swing.reversedFrom.includes('0.5')

                                const sevenCount = reversedLevels.filter(e => e === '0.786').length
                                const sixCount = reversedLevels.filter(e => e === '0.618').length
                        if (swing.start === 47188.1 && time === '9:36:00 am') {
                                // count++
                                // console.log(swing.reversedFrom, currentSwingCondition)
                        }

                                if (reversedLevels.includes('0.786') && lastReverseFrom != '0.236' && reversedLevels.length < 5 && !currentSwingCondition
                                        &&
                                        (sevenCount > sixCount)
                                        &&
                                        (swing.reversedFrom.length === 1 ? swing.reversedFrom[1] === '0.618' : true)
                                ) {
                                // if (reversedLevels.includes('0.786') && lastReverseFrom != '0.236' && reversedLevels.length < 10 && swingLastReverseFrom != '0.618') {
                                        console.log(reversedLevels, swing.reversedFrom)
