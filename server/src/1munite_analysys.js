...
const smaPrices = newData.map(e => +e.price)
const period = 2000

const EMA = indicatorts.ema(smaPrices, { period })
console.log(EMA[100], smaPrices[100])

newData.forEach((e, index) => {
        const emaTrend = EMA[index] > +e.price ? DOWN : UP
        findSwings(+e.price, e.timestamp, emaTrend)
        checkFibLabelTouch(+e.price, e.timestamp, emaTrend)
...
console.log(`Reversed from 0.786: `, notReveseList.filter(e => e.reversed && !e.reEntry).length)
